...
class HttpRequest {
    ...
    fun makeServiceCall(reqUrl: String?, token: String?=null): String? {         var response: String? = null
        try {
            val url = URL(reqUrl)
            val conn = url.openConnection() as HttpURLConnection             if (token != null)
            {
    ...
    {
        Log.i(TAG, "convertStreamToString: $line")             e.printStackTrace()
    }         finally {             try {
        `is`.close()
    }
    catch (e: IOException)
